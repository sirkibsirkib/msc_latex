...
Reo is a graphic language which represents a system as a \textit{connector}, a multi-graph which defines the relationships between its nodes. Nodes represent logical `locations' which may observe a single datum at a time. Ultimately, a connector's relations boil down to Reo's underlying semantics, of which there are several. They have in common that they constrain the synchronous observations of data at nodes. For example, a connector may enforce that when node~$A$ observes an element~$X$, node~$B$ observes~$X$ also. In this way, connectors specify the ways in which data is permitted to synchronously `flow' through its nodes. Literature often uses \textit{circuit} as an alternative name for connectors, drawing on the appropriate metaphor of electricity, or perhaps fluid pressure as it appears to propagate movement forward and blockages backward synchronously. 

A tenet of Reo's design philosophy is compositionality (and consequently, modularity). Rather than defining each and every connector anew in terms of the underlying semantics, all but the simplest primitive connectors are built from simpler ones. At each level of this hierarchy, the connector exposes a subset of its nodes to the connector above, wherein they are called \textit{ports}. The metaphor evokes the transmission of data a harbor to the wider world. Appropriately, the set of a connectors ports is called its \textit{interface}, beyond which is the connector's \textit{environment}. A complex connector emerges when it links the ports of its simpler, constituent connectors. Reo is made more applicable to the world of computing by defining ports with a fixed \textit{orientation}; \textit{input} ports represent data coming from the environment, and \textit{output} ports are the opposite. With these annotations, it becomes possible to interpret a synchronous \textit{interaction} involving a set of oriented ports as the synchronous propagation of data from sources to sinks. Non-port nodes are \textit{internal}, and are not accessible from the environment. Connectors with empty interfaces are \textit{closed}, and can be understood as `complete' systems. 

Some Reo primitives (and consequently, some connectors that incorporate them) define relationships in terms of variables whose states can persist into the future. In this way, Reo can model \textit{asynchronous} data flow also, breaking up contiguous networks of synchrony, and constraining future observations in terms of its changing state. The primitive \textit{fifo1} connector is canonical for this purpose; in observing some datum~$X$ on its single input port, an \textit{empty} connector becomes \textit{full}, whereafter it may again become empty by observing~$X$ on its single output port\footnote{The initially-full variant of \textit{fifo1} allows Reo to express connectors initialized with any imaginable initial state.}. Despite its simplicity, \textit{fifo1} is sufficient for the expression of arbitrary asynchronous events and storing arbitrary state.

...
In the literature, Reo is usually expressed using graphical syntax, representing nodes as one would expect. Building a connector graphically involves drawing connections between nodes as \textit{edges} for the usual binary primitives (\textit{sync}, \textit{fifo1}, etc.), or as \textit{boxes}, exposing ports at their boundaries. In this work, we focus instead on Reo's \textit{textual} syntax instead, as it makes for a more practical interface between human and compiler. In this context, Reo is fundamentally the same, but relies on textual \textit{identifiers} for nodes and connectors as is typical for general-purpose programming languages. Listing~\ref{fig:alternator} gives an example of the \textit{alternator2}, a canonical example connector, just complex enough to demonstrate the composition of primitives.


\begin{listing}[h!]
	\centering
	\inputminted{text}{alternator2.treo}
	\caption[Textual Reo specification of alternator2.]{Textual Reo specification of the \textit{alternator2} connector with input ports~$\{A,B\}$, and output port~$C$. $$}
	\label{listing:alternator2_treo}
\end{listing}

...
The notion of `affinity' as we use it has its roots in classical and intuitionistic logic. In a type-theoretic proof system, one can attempt to derive some \textit{judgment}$\Gamma\vdash{}t:\tau$ with \textit{statements} assigning \textit{types} to \textit{terms} (here term $t$ is assigned type $\tau$) under context $\Gamma$. A context is simply a list of statements, which can be thought to correspond with \textit{assumptions} or \textit{premises}. The jdugment holds if one can construct a proof, starting with the judgment and select and applying rules until no `dangling' judgments remain. For example, simply-typed lambda calculus has a type-derivation rule for abstraction, substitution and application~\cite{nederpelt2014type}. Depending on the type system, \textit{structural rules} may additionally be provided for manipulating (`massaging') the context such that other rules may be applied. For example, \textit{weakening} is a common structural rule that allows one to arbitrarily discard statements from the context. Depending on the proof system, this may be useful or even necessary before other rules can be applied such that the proof can be completed. For example, $weakening$ is required to prove $A, t:\tau\vdash t:\tau$, as $var$ cannot be directly applied. One can imagine that the ability to arbitrary replicate, discard and re-arrange terms effectively describes a type system that treats its context like a set.
\[
\begin{aligned}
...
\textit{Affine} type systems are characterized by the absence of the \textit{contraction} rule. Proofs cannot replicate statements at will, and thus they must be treated as a finite resource in the proof. 
Affine type systems correspond to this domain of logic. It is fruitful to think of logical statements as finite resources which are \textit{consumed} when involved in operations~\cite{walker2005substructural}.

...
Classical or intuitionistic logic systems have structural rules for transforming judgements or sequents. These transformations are performed at the `meta' level, re-formulating the formula, and are thus only applicable in situations which do not add meaning to the formula (otherwise, the transformation would not be correct). One such structural rule is \textit{contraction}, in which two equal or unifiable members on the same side of a sequent are replaced by the instance they have in common. As a rule:
\[\frac{\Gamma\vdash{}A,A,\Sigma}{\Gamma\vdash{}A,\Sigma}\]
\hl{ask hans for a nice intro here and citation}
Affine logic is a structural logic in which contraction is absent. In this setting, one cannot apply \textit{idempotency of entailment}. Affine type systems correspond to this domain of logic~\cite{walker2005substructural}. The logical inability to contract terms manifests as the inability to arbitrarily duplicate values. It is fruitful to think of values here as finite resources which are \textit{consumed} when involved in operations. Affine logic offers a way to model the limitations inherent to resource access such that they can be formally specified and reliably enforced. As type systems do in general, they introduce limitations on the programmer. Of course, the hardware has no problem replicating the bytes representing some integer. Why then do we limit ourselves? This argument can be made for type systems in general; the machine likewise has no problem re-interpreting the bytes storing a string as an integer. The benefits of adding these artificial constraints are to reduce the possibility space of programs such that the majority of those \textit{prohibited} by the type system are `bad'. It is common paradigm to dedicate a memory region to one kind of object for the duration of the program, making it easier to understand the program flow for programmers. Furthermore, the addition of explicit types communicates \textit{intent} to the compiler which can be readily exploited; it is common practice for compilers to perform extensive transformation on the resulting binary which preserve only the programmer's intent. 

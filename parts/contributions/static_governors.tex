...
%The design of the governor based on type-state automata discussed thus-far are able to accurately represent the configuration space of \textit{approximated} RBAs, as explained in Section~\ref{sec:approximating_rba}. The design so far also \textit{obliges} the end-user to walk through the projected, normalized automaton in a predefined manner. This is a consequence of the governor removing the compute components' ability to \textit{make decisions} that impact the protocol such that the system reaches consensus, as described in Section~\ref{sec:rule_consensus}. Consider a compute component which interfaces using ports $\{A,B\}$ with a protocol defined by RBF in Table~\ref{tab:counting_rbf}. Generating a governor for this RBA results in a new projected and normalized RBA, shown compared to the original RBA in Figure~\ref{fig:counter_RBAs}. As always, normalization does not change the sequence of ports on which the component must operate, in this case being an infinitely-repeating chain of $[A,A,A,A,B]$. The state space cannot be reduced without losing information; States `000' and `001' have in common that the next several port operations are $A$, but they are still distinguishable. The user must treat these states differently as they are not interchangeable, and thus, do not correspond to interchangeable types. Imagine the user's experience in the case of the similar \textit{512-count cycle} protocol, where their component must distinguish between 512 states!
%
%
%
%\begin{table}
...
%		& $\gapwedge{}m_0'=d_A$\\
%		1 & $m_0\neq{}*\wedge{}m_1=*$
%		& $\gapwedge{}m_1'=m_0\wedge{}m_0'=*$\\
%		2 & $m_0=m_1\neq{}*\wedge{}m_2=*$
%		& $\gapwedge{}m_2'=m_0\wedge{}m_0'=m_1'=*$\\
%		3 & $m_0=m_1=m_2\neq{}*$
%		& $\gapwedge{}d_B=m_0\wedge{}m_0'=m_1'=m_2'=*$\\
%		\hline
%	\end{tabular}
%	\caption{RBF of the \textit{8-count cycle} connector which uses the full/empty states of its memory cells $m_{0-2}$ as bits for counting in base~2. Rule~3 rolls over to zero and puts a value to port $B$. Every second rule fired is~0, getting a value from port~$A$.}
